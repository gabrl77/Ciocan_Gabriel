...
def genereaza_nume():
    ...
    if random.random() > 0.5:
        return random.choice(prenume_f) + " " + random.choice(nume)
    else:
        return random.choice(prenume_m) + " " + random.choice(nume)
...
class HashTableGUI:
    ...
    def setup_ui(self):
        ...
        tk.Label(self.root, text="Introduceti CNP (13 cifre):", font=('Arial', 12)).pack(pady=5)
    ...
    def cauta(self):
        ...
        if len(cnp) != 13 or not cnp.isdigit():
            messagebox.showerror("Eroare", "CNP invalid! Trebuie sa contina exact 13 cifre.")
            return
